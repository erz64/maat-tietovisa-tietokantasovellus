...
def get_highscores_all(question):
    sql = """SELECT username, MAX(score) FROM SCORES WHERE question=:question
            GROUP BY username ORDER BY MAX(SCORE) DESC LIMIT 3"""
    highscores = db.session.execute(sql, {"question": question}).fetchall()
    return highscores


def insert_into_scores(user_id, question, username, score):
    sql = """INSERT INTO SCORES (user_id, question, username, score)
            VALUES (:user_id, :question, :username, :score)"""
...
def get_reviews_all(question_id):
    sql = """SELECT AVG(star) FROM reviews
...
def insert_into_reviews(user_id, star, question_id):
    sql = """INSERT INTO reviews (user_id, star, question_id)
            SELECT :user_id, :star, :question_id WHERE NOT EXISTS (SELECT user_id FROM reviews
