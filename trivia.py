...
def upload_images_and_questions(name, question_answer, id, data, question):
    sql_question = """INSERT INTO questions (question_id, question, answer)
                        VALUES (:question_id, :question, :question_answer)"""
    sql_image = """INSERT INTO images (question_id,question_answer,name,data)
                    VALUES (:question_id,:question_answer,:name,:data)"""
    ...
    db.session.execute(sql_image, {
        "question_id": id, "question_answer": question_answer,
        "name": name, "data": data})
    db.session.commit()
...
def get_picture(question_answer, question_id):
    sql = """SELECT encode(data, 'base64') FROM images WHERE
            question_answer =:question_answer AND question_id =:question_id"""
    result = db.session.execute(
        sql, {"question_answer": question_answer, "question_id": question_id})
...
def get_random_question(question_id):
    ...
    while True:
        ...
        answer_sql = """SELECT answer FROM questions WHERE
                        question_id =:question_id LIMIT 1 OFFSET :position"""
        answer = db.session.execute(
            answer_sql, {"question_id": question_id, "position": position}).fetchone()[0]
        if answer in session['asked']:
            continue
        question_sql = """SELECT question FROM questions WHERE
                            question_id =:question_id LIMIT 1 OFFSET :position"""
